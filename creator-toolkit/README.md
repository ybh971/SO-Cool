...
一个 Claude Code 技能包，帮内容创作者完成**完整的内容生产管线**：
...
你说："/script-writer 果冻甲测评 30秒"
  ↓ AI 生成时间轴+画面+台词+备注四列口播脚本
...
| Skill | 做什么 | 输入 | 输出 |
|-------|--------|------|------|
| `/topic-finder` | 实时搜索选题 | 领域 + 数量 | 3-5个选题卡片 |
| `/content-adapter` | 多平台文案适配 | 选题 + 平台 | 抖音/小红书/B站文案 |
| `/script-writer` | 口播脚本生成 | 选题 + 时长 | 时间轴四列脚本 |
...
### 方式一：直接复制（推荐，零门槛）
1. 下载这个文件夹
2. 把 `skills/` 里的文件复制到你当前项目的 `.claude/skills/` 目录
...
**Q: script-writer 支持哪些时长？**
A: 15秒、30秒、60秒、3分钟、5分钟。会根据时长自动调整节奏和台词密度。

**Q: 能支持更多平台吗？**
A: 你可以自己在 `templates/` 里加新平台的格式规范，skills 会自动读取。
